...
1. `git pull` on the current build-track branch (currently `{{BUILD_TRACK_BRANCH}}`).
...
<!-- Replace this table with YOUR project's critical work types -->

{{MANDATORY_ROUTING}}

All other work (CRUD, schemas, routers, migrations, hooks, UI, tests, docs, seed scripts) also routes through Architect. Prefer **Plan-Review** over improvising.
...
27. Quick commits, pushes, and opens a PR from the step branch to `{{BUILD_TRACK_BRANCH}}`. **Quick does not merge the PR.**
...
<!-- Replace with YOUR project's file locations -->
